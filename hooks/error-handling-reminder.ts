...
function main() {
    try {
        ...
        if (categories.backend.length > 0) {
            ...
            const hasTryCatch = backendFiles.some(
                (f) => f.analysis.hasTryCatch,
            );
            const hasPrisma = backendFiles.some((f) => f.analysis.hasPrisma);
            const hasController = backendFiles.some(
                (f) => f.analysis.hasController,
            );
        ...
        if (categories.frontend.length > 0) {
            ...
            const hasTryCatch = frontendFiles.some(
                (f) => f.analysis.hasTryCatch,
            );
